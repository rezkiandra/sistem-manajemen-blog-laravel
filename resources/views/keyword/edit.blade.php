...
@section('content')
  <div class="row">
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          <h4 class="card-title mb-4">{{ __('Edit Data Keyword') }}</h4>

          {{-- <x-error-handling /> --}}
          <x-create-form :action="route('keyword.update', $keyword)">
            @csrf
						@method('PUT')
            <x-input-label :label="'Nama Keyword'" :name="'name'" :class="'tagify'" :placeholder="'Masukkan keyword'" :value="$keyword->name" />
          </x-create-form>
        </div>
      </div>
    </div>
